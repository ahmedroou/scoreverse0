      ...
      {overallScores.length === 0 ? (
         <Card className="text-center py-12 bg-card border-border shadow">
          <CardHeader>
            ...
            <CardTitle className="text-2xl text-card-foreground">{t('share.noMatches')}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-muted-foreground">
              {t('share.noMatchesDescription', {spaceName: data.space.name})}
            </p>
          </CardContent>
        </Card>
      ) : (
        <Tabs defaultValue="overall" className="w-full">
          ...
          {relevantGames.map(game => (
            <TabsContent key={game.id} value={game.id}>
              <LeaderboardTable scores={getGameLeaderboard(game.id)} title={`${t('leaderboards.gameLeaderboard', {gameName: game.name})} (${data.space.name})`} isPublicView />
            </TabsContent>
          ))}
        </Tabs>
      )}
