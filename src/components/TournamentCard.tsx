...
export function TournamentCard({ tournament, game, leader, ownerUsername, isPublicView = false }: TournamentCardProps) {
  ...
  const CardContentComponent = (
    <Card className={`border hover:shadow-lg transition-all duration-200 ease-in-out bg-card flex flex-col h-full ${tournament.status === 'completed' ? 'border-yellow-500/30 bg-yellow-500/5' : 'border-border'} ${isPublicView ? '' : 'hover:scale-[1.02] hover:border-primary'}`}>
